...
interface RunSummary { id: string; runName: string; status: RunStatus; executionMode: ExecutionMode; dataOrigin: DataOrigin; caseCount: number; generatedTasks: number; totalGenerationTasks: number; candidateCount: number; selectedCount: number; primaryReviewCount: number; pairwiseReviewCount: number; adjudicationCount: number; createdAt: string; updatedAt: string; }
interface InitialState { user: PublicUser | null; storageMode: string; cases: EvaluationCase[]; promptVersions: PromptVersion[]; users: PublicUser[]; runs: RunSummary[]; }
...
export function EvaluationClient({
  ...
  const canManage = initial.user?.role === "admin";
  ...
  return (
    <div className="min-h-screen">
      {adminNavigation ? <AdminWorkspaceHeader {...adminNavigation} /> : <AppHeader />}
      <main className="mx-auto w-full max-w-7xl px-4 py-7 pb-20 md:px-6">
        {adminNavigation && <AdminBackLink href="/admin" label="返回数据看板" />}
        <header className={`${adminNavigation ? "mt-5 " : ""}flex flex-col gap-5 border-b border-[var(--color-line-strong)] pb-6 md:flex-row md:items-end md:justify-between`}>
          <div><p className="flex items-center gap-2 text-xs font-black text-[var(--color-accent)]"><Flask size={16} weight="bold" />离线人工评测 · {formatDataOrigin("evaluation_set")}</p><h1 className="mt-3 text-3xl font-black tracking-[-0.04em] sm:text-4xl">Prompt 升级证据台</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--color-graphite)]">固定输入、同模型参数、双人独立评分和持久化盲评，共同决定候选版本是否值得升级。</p></div>
          <div className="flex items-center gap-2"><span className="control-base inline-flex min-h-10 items-center gap-2 px-3 text-xs font-bold"><Database size={15} />{initial.storageMode === "postgres" ? "PostgreSQL 正式存储" : "本地 JSON 存储"}</span>{initial.user ? <button className="button-secondary" onClick={logout} type="button"><SignOut size={16} />退出</button> : <Link className="button-secondary" href="/evaluation/login?next=%2Fevaluation"><SignOut size={16} />管理员登录</Link>}</div>
        </header>

        {!initial.user && <p className="mt-5 rounded-[10px] bg-[var(--color-surface-subtle)] px-4 py-3 text-xs font-bold text-[var(--color-muted)]">当前为公开只读模式，可查看评测批次与汇总进度；创建批次、账号管理和评分操作仍需登录。</p>}
        ...
        <section className="editorial-panel mt-6 overflow-hidden"><div className="border-b border-[var(--color-line)] p-5"><h2 className="text-sm font-black">评测批次</h2><p className="mt-1 text-xs text-[var(--color-muted)]">Mock 批次永久显示“需要继续评测”，不会形成升级结论。</p></div>
          {initial.runs.length === 0 ? <div className="p-10 text-center text-sm text-[var(--color-muted)]">暂无批次。管理员先创建评测账号和第一个批次。</div> : <div className="divide-y divide-[var(--color-line)]">{initial.runs.map((run) => <Link className="grid gap-3 p-5 transition-colors hover:bg-[var(--color-surface-subtle)] sm:grid-cols-[1fr_auto] sm:items-center" href={`/evaluation/runs/${run.id}`} key={run.id}><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-black">{run.runName}</h3><span className="rounded-full bg-[var(--color-surface-subtle)] px-2 py-1 text-[10px] font-black">{formatExecutionMode(run.executionMode)}</span><span className="text-[10px] font-bold text-[var(--color-muted)]">{formatRunStatus(run.status)}</span></div><p className="mt-2 text-xs text-[var(--color-muted)]">生成 {run.generatedTasks}/{run.totalGenerationTasks} · 候选 {run.candidateCount}/360 · 正式结果 {run.selectedCount}/{run.caseCount * 2} · 原始评分 {run.primaryReviewCount}/{run.caseCount * 4}</p></div><ArrowRight size={18} weight="bold" /></Link>)}</div>}
        </section>

        {canManage && <div className="mt-6 grid gap-4 lg:grid-cols-2"><section className="editorial-panel p-5"><h2 className="text-sm font-black">固定案例</h2><p className="mt-2 text-xs text-[var(--color-muted)]">{initial.cases.length} 条 · {new Set(initial.cases.map((item) => item.topicId)).size} 个主题 · 输入在批次中不可修改</p><div className="mt-4 flex flex-wrap gap-2">{['小红书','抖音','B站'].map((label) => <span className="control-base px-3 py-2 text-xs font-bold" key={label}>{label} {initial.cases.filter((item) => item.platformLabel === label).length}</span>)}</div></section><section className="editorial-panel p-5"><h2 className="text-sm font-black">Prompt 版本</h2><div className="mt-4 space-y-3">{initial.promptVersions.map((prompt) => <details className="control-base p-3" key={prompt.id}><summary className="cursor-pointer text-xs font-black">{prompt.version} · {formatPromptRole(prompt.role)}<span className="ml-2 font-normal text-[var(--color-muted)]">{prompt.changeSummary}</span></summary><pre className="mt-3 max-h-48 overflow-auto whitespace-pre-wrap text-[11px] leading-5 text-[var(--color-graphite)]">{prompt.promptContent}</pre>{prompt.role === 'candidate' && <button className="button-secondary mt-3" onClick={() => setBaseline(prompt.id)} type="button">设为当前基准版本</button>}</details>)}</div><form className="mt-5 space-y-3 border-t border-[var(--color-line)] pt-5" onSubmit={createPrompt}><p className="text-xs font-black">创建不可变候选版本</p><div className="grid grid-cols-2 gap-3"><input className="control-base min-h-10 px-3 text-sm" name="version" placeholder="v1.2" required /><input className="control-base min-h-10 px-3 text-sm" name="name" placeholder="版本名称" required /></div><input className="control-base min-h-10 w-full px-3 text-sm" name="changeSummary" placeholder="修改说明" required /><textarea className="control-base min-h-28 w-full p-3 text-sm" name="promptContent" placeholder="完整 Prompt 文本" required /><button className="button-secondary w-full" type="submit">创建新版本（不覆盖历史）</button></form></section></div>}
      </main>
    </div>
  );
}
