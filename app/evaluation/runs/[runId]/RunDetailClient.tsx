...
export function RunDetailClient({ initialRun, user, adminNavigation }: { initialRun: EvaluationRunRecord; user: PublicUser | null; adminNavigation?: AdminNavigationFlags }) {
  const [run, setRun] = useState(initialRun);
  const [report, setReport] = useState<ReadableEvaluationReport | null>(null);
  ...
  const generated = user ? run.generationTasks?.filter((item) => item.terminalStatus === "success").length ?? 0 : Number((run as unknown as { generatedTasks?: number }).generatedTasks ?? 0);
  const totalTasks = user ? run.generationTasks?.length ?? run.caseCount * 2 : Number((run as unknown as { totalGenerationTasks?: number }).totalGenerationTasks ?? run.caseCount * 2);
  const pairwiseDecisionCount = user && run.pairwiseDecisions
    ? run.pairwiseDecisions.filter((item) => item.winnerRole).length
    : Number((run as unknown as { pairwiseDecisionCount?: number }).pairwiseDecisionCount ?? 0);
  const reportSummary = report ? summarizeEvaluationReport(report) : null;
  ...
  return (
    <div className="min-h-screen">{adminNavigation ? <AdminWorkspaceHeader {...adminNavigation} /> : <AppHeader />}<main className="mx-auto w-full max-w-7xl px-4 py-7 pb-20 md:px-6">
      ...
      <header className="mt-5 flex flex-col gap-4 border-b border-[var(--color-line-strong)] pb-6 md:flex-row md:items-end md:justify-between"><div><p className="flex items-center gap-2 text-xs font-black text-[var(--color-accent)]"><Flask size={15} weight="bold" />{formatDataOrigin(run.dataOrigin)} · {formatExecutionMode(run.executionMode)}</p><h1 className="mt-3 text-3xl font-black tracking-[-0.04em]">{run.runName}</h1><p className="mt-2 text-xs text-[var(--color-muted)]">状态：{formatRunStatus(run.status)} · {run.caseCount} 个固定案例 · 当前身份：{user ? `${user.displayName}（${formatUserRole(user.role)}）` : "公开访客（只读）"}</p></div>{(user?.role === 'admin' || !user) && <button className="button-secondary" onClick={loadReport} type="button">查看升级报告</button>}</header>

      <section className="mt-6 grid overflow-hidden rounded-[14px] border border-[var(--color-ink)] bg-white sm:grid-cols-4">{[
        ['生成任务',`${generated}/${totalTasks}`],['候选结果',`${user ? run.candidates?.length ?? 0 : Number((run as unknown as { candidateCount?: number }).candidateCount ?? 0)}/${run.caseCount * 6}`],['正式结果',`${user ? run.formalResults?.length ?? 0 : Number((run as unknown as { selectedCount?: number }).selectedCount ?? 0)}/${run.caseCount * 2}`],['A/B 定案',`${pairwiseDecisionCount}/${run.caseCount}`]
      ...
      {user?.role === 'admin' && <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <section className="editorial-panel p-5"><h2 className="text-sm font-black">1. 批量生成</h2><p className="mt-2 text-xs leading-5 text-[var(--color-muted)]">每次处理一个案例的基准版本和候选版本，共产生 6 条候选；失败状态保留在原始证据中。</p><div className="mt-5 grid grid-cols-2 gap-2"><button className="button-secondary" disabled={busy || generated >= totalTasks || run.generationTasks?.some((item) => item.terminalStatus === 'format_error' || item.terminalStatus === 'generation_error')} onClick={() => action({ action: 'generate-next' })} type="button">生成下一案例</button><button className="button-primary" disabled={busy || generated >= totalTasks || run.generationTasks?.some((item) => item.terminalStatus === 'format_error' || item.terminalStatus === 'generation_error')} onClick={generateAll} type="button"><Play size={16} weight="fill" />{busy ? '批量处理中' : generated >= totalTasks ? '生成已完成' : '连续生成全部'}</button></div>{run.generationTasks?.filter((item) => item.terminalStatus === 'format_error' || item.terminalStatus === 'generation_error').map((task) => <div className="mt-3 rounded-[8px] bg-[var(--color-danger-soft)] p-3 text-xs" key={task.id}><p className="font-bold text-[var(--color-danger)]">{task.caseId} · {formatPromptRole(task.promptRole)} · {formatGenerationTaskStatus(task.terminalStatus)}</p><p className="mt-1 text-[var(--color-muted)]">{formatGenerationErrorCategory(task.lastError)}</p><button className="button-secondary mt-2" onClick={() => action({ action: 'retry-generation', taskId: task.id })} type="button">重新排队</button></div>)}</section>
      ...
      {report && reportSummary && (
        <section className="editorial-panel mt-6 overflow-hidden" aria-labelledby="evaluation-report-heading">
          <div className="border-b border-[var(--color-line)] p-5">
            <h2 className="text-sm font-black" id="evaluation-report-heading">评测结论报告</h2>
            <p className="mt-2 text-xs leading-5 text-[var(--color-muted)]">结论仅基于固定离线评测集，不代表真实点击、发布或传播效果。</p>
          </div>
          <div className="grid border-b border-[var(--color-line)] lg:grid-cols-[1.2fr_0.8fr]">
            <div className="p-5">
              <p className="text-[11px] font-bold text-[var(--color-muted)]">当前结论</p>
              <p className="mt-2 text-2xl font-black">{reportSummary.recommendationLabel}</p>
              <p className="mt-3 text-xs leading-5 text-[var(--color-graphite)]">{reportSummary.reason}</p>
              {!reportSummary.conclusive && <p className="mt-3 rounded-[8px] border border-[var(--color-warning)] bg-[var(--color-warning-soft)] px-3 py-2 text-xs font-bold text-[var(--color-warning)]">当前证据不足，不可据此执行升级或启用。</p>}
            </div>
            <dl className="grid grid-cols-3 border-t border-[var(--color-line)] lg:grid-cols-1 lg:border-l lg:border-t-0">
              <div className="p-4"><dt className="text-[10px] font-bold text-[var(--color-muted)]">证据类型</dt><dd className="mt-1 text-xs font-black">{formatExecutionMode(run.executionMode)}</dd></div>
              <div className="border-l border-[var(--color-line)] p-4 lg:border-l-0 lg:border-t"><dt className="text-[10px] font-bold text-[var(--color-muted)]">批次状态</dt><dd className="mt-1 text-xs font-black">{formatRunStatus(run.status)}</dd></div>
              <div className="border-l border-[var(--color-line)] p-4 lg:border-l-0 lg:border-t"><dt className="text-[10px] font-bold text-[var(--color-muted)]">当前门禁结果</dt><dd className="mt-1 text-xs font-black">{reportSummary.conclusive ? `${reportSummary.passedGateCount}/${reportSummary.totalGateCount}` : '待完成'}</dd></div>
            </dl>
          </div>
          <div className="grid border-b border-[var(--color-line)] sm:grid-cols-3">
            {reportSummary.metrics.map((metric) => <div className="border-b border-[var(--color-line)] p-4 last:border-b-0 sm:border-b-0 sm:border-r" key={metric.label}><p className="text-[10px] font-bold text-[var(--color-muted)]">{metric.label}</p><p className="mt-2 text-xl font-black tabular-nums">{metric.value}</p></div>)}
          </div>
          <div className="p-5">
            <h3 className="text-xs font-black">七项决策门禁</h3>
            <div className="mt-3 divide-y divide-[var(--color-line)] border-y border-[var(--color-line)]">
              {reportSummary.gates.map((gate) => <div className="grid gap-1 py-3 text-xs sm:grid-cols-[1fr_auto] sm:items-center" key={gate.key}><div><p className="font-bold">{gate.label}</p>{gate.threshold && <p className="mt-1 text-[10px] text-[var(--color-muted)]">门槛：{gate.threshold}{typeof gate.actual === 'number' ? ` · 当前值：${gate.actual}` : ''}</p>}</div><span className="font-black text-[var(--color-graphite)]">{reportSummary.conclusive ? gate.passed ? '通过' : '未通过' : '待验证'}</span></div>)}
            </div>
            {user?.role === 'admin' && <div className="mt-5 flex flex-wrap gap-2">{['evaluation_cases.csv','evaluation_generations.csv','human_evaluations.csv','pairwise_evaluations.csv','bad_cases.csv','evaluation_report.json','evaluation_report.md'].map((file) => <a className="button-secondary" href={`/api/evaluation/runs/${run.id}/export?file=${file}`} key={file}>{file}</a>)}</div>}
            <details className="mt-4 border-t border-[var(--color-line)] pt-4"><summary className="cursor-pointer text-xs font-bold">查看原始 JSON</summary><pre className="mt-3 max-h-[480px] overflow-auto bg-[var(--color-surface-subtle)] p-4 text-xs leading-6">{JSON.stringify(report, null, 2)}</pre></details>
          </div>
        </section>
      )}
    </main></div>
